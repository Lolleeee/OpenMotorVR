...
public class Hotbar : MonoBehaviour
{
    ...
    void CreateVisibleGrid()
    {
        ...
        for (int row = 0; row < visibleRows; row++)
        {
            for (int col = 0; col < visibleColumns; col++)
            {
                ...
                BoxCollider boxCol = itemObj.AddComponent<BoxCollider>();
                boxCol.size = new Vector3(1, 1, 0.01f);
                
                // Add XR Simple Interactable
                itemObj.AddComponent<UnityEngine.XR.Interaction.Toolkit.Interactables.XRSimpleInteractable>();
    ...
    void UpdateHotbarPosition()
    {
        ...
        activeHotbar.transform.rotation = attachTransform.rotation;
    }
    ...
    void HandleNavigation()
    {
        ...
        if (navigationCooldown <= 0 && navigationInput.magnitude > 0.5f)
        {
            int oldRow = currentVirtualRow;
            int oldCol = currentVirtualCol;
            
            // Navigate in virtual grid
            if (navigationInput.x > 0.5f) // Right
            {
                currentVirtualCol++;
                if (currentVirtualCol >= visibleColumns)
                {
                    currentVirtualCol = 0;
                    currentVirtualRow++;
                }
            ...
            else if (navigationInput.y > 0.5f) // Up
            {
                currentVirtualRow--;
            }
    ...
    void SpawnSelectedItem()
    {
        ...
        if (actualIndex >= 0 && actualIndex < totalItems)
        {
            ...
            {
                Transform attachTransform = nearFarInteractor.attachTransform;
                Vector3 spawnPosition = attachTransform.position + attachTransform.TransformDirection(spawnOffset);
